...
elo = json.load(open("./data/elo-calibrated.json"))["ratings"]
wc = json.load(open("./data/wc2026-results.json"))["matches"]
backtest = json.load(open("./data/model-backtest.json"))
prob_live = json.load(open("./data/probabilities.json"))
prob_pre = json.load(open("./data/probabilities-pre.json"))
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .mode-btn {{
      background: #080c16; border: 1px solid rgba(255, 255, 255, 0.15); color: var(--text-muted);
      padding: 10px 18px; border-radius: 10px; font-family: var(--font-mono); font-size: 0.85rem; font-weight: 700;
      cursor: pointer; transition: all 0.2s;
    }}

    .mode-btn.active {{
      background: var(--neon-gold); color: #000; border-color: var(--neon-gold); font-weight: 800;
    }}
    ...
    .stage-pill-high {{ background: rgba(255, 183, 0, 0.2); color: var(--neon-gold); border: 1px solid rgba(255, 183, 0, 0.4); padding: 4px 10px; border-radius: 6px; font-weight: bold; }}
    .stage-pill-mid {{ background: rgba(0, 242, 254, 0.15); color: var(--neon-cyan); border: 1px solid rgba(0, 242, 254, 0.3); padding: 4px 10px; border-radius: 6px; font-weight: bold; }}
    .stage-pill-low {{ background: rgba(255, 255, 255, 0.05); color: var(--text-main); padding: 4px 10px; border-radius: 6px; }}
...
<body>
  ...
  <div class="marquee-container">
    <div class="marquee-inner">
      ...
      <span class="marquee-item"><span class="marquee-tag">PRE-TOURNAMENT FAVORITE</span> SPAIN 22.4% · ARGENTINA 20.1% · FRANCE 11.5%</span>
  ...
  <div class="wrapper">
    ...
    <header>
      ...
      <p class="header-sub">
        Full 48-team, 104-match model powered by Elo ratings, Dixon-Coles bivariate Poisson goal distributions, and 50,000 Monte Carlo tournament simulations. Both pre-tournament baseline and live-conditioned modes available.
      </p>
    </header>
    ...
    <div class="hero-metrics">
      <div class="metric-box">
        <div class="metric-label-txt">Pre-Tournament Fav</div>
        <div class="metric-big-num" style="color: var(--neon-gold)">Spain</div>
        <div class="metric-tag-pill pill-gold">22.4% CHAMPION ODDS</div>
      </div>
      <div class="metric-box">
    ...
    <div id="tab-title-race" class="tab-content active">
      ...
      <div class="contenders-grid">
        <div class="contender-card">
          ...
          <div class="contender-prob">22.4%</div>
          <div style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--neon-cyan); margin-top: 4px;">PRE-TOURNAMENT CHAMPION ODDS</div>
        </div>
        <div class="contender-card" style="border-color: rgba(0, 242, 254, 0.3);">
          ...
          <div class="contender-prob" style="color: var(--neon-cyan);">20.1%</div>
          <div style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--neon-cyan); margin-top: 4px;">PRE-TOURNAMENT CHAMPION ODDS</div>
        </div>
        <div class="contender-card" style="border-color: rgba(255, 0, 127, 0.3);">
          <div class="contender-rank">#3</div>
          <div class="contender-name">France</div>
          <div class="contender-prob" style="color: var(--neon-magenta);">11.5%</div>
          <div style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--neon-magenta); margin-top: 4px;">PRE-TOURNAMENT CHAMPION ODDS</div>
        </div>
        <div class="contender-card" style="border-color: rgba(0, 255, 135, 0.3);">
          <div class="contender-rank">#4</div>
          <div class="contender-name">Brazil</div>
          <div class="contender-prob" style="color: var(--neon-green);">11.1%</div>
          <div style="font-family: var(--font-mono); font-size: 0.85rem; color: var(--neon-green); margin-top: 4px;">PRE-TOURNAMENT CHAMPION ODDS</div>
        </div>
      </div>
      ...
      <div class="table-card" style="padding: 28px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 16px;">
          <div>
            ...
            <p style="color: var(--text-muted); font-size: 0.85rem; margin-top: 4px;">Switch between unconditioned pre-tournament predictions and live in-tournament conditioned results.</p>
          </div>
          
          <div class="mode-toggle-bar">
            <button class="mode-btn active" id="btn-mode-pre" onclick="setTitleMode('pre')">🔮 Pre-Tournament Probabilities</button>
            <button class="mode-btn" id="btn-mode-live" onclick="setTitleMode('live')">🔴 Live In-Tournament Locked</button>
          </div>
        </div>

        <div style="margin-bottom: 16px;">
          <input type="text" id="title-search" placeholder="Search nation..." oninput="renderTitleRaceTable()" style="width: 280px;">
        </div>
    ...
    <div id="tab-methodology" class="tab-content">
      <div class="table-card" style="padding: 32px;">
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
          <a href="data/probabilities.json" download class="btn-action">📥 DOWNLOAD LIVE PROBABILITIES.JSON</a>
          <a href="data/probabilities-pre.json" download class="btn-action" style="background: var(--neon-cyan)">📥 DOWNLOAD PRE-TOURNAMENT PROBABILITIES.JSON</a>
          <button onclick="exportCSV()" class="btn-action" style="background: var(--neon-magenta); color: #fff;">📊 EXPORT TO CSV</button>
        </div>
      </div>
    </div>
  ...
  <script>
    const ELO = {json.dumps(elo)};
    const WC = {json.dumps(wc)};
    const BACKTEST = {json.dumps(backtest)};
    const PROB_LIVE = {json.dumps(prob_live)};
    const PROB_PRE = {json.dumps(prob_pre)};
    const GROUPS = {json.dumps(groups_def)};

    const DC_RHO = -0.13;
    const selectedGroupAdv = {{}};
    let titleRaceMode = 'pre'; // 'pre' or 'live'
    ...
    // RENDER TITLE RACE PROBABILITIES DYNAMICALLY
    function renderTitleRaceTable() {{
      ...
      const dataset = (titleRaceMode === 'pre' ? PROB_PRE.teams : PROB_LIVE.teams);
      ...
      const fmtCell = (val) => {{
        if (titleRaceMode === 'live') {{
          if (val === 1) return '<span class="stage-pill-100">100% (Passed)</span>';
          if (val > 0) return '<span class="stage-pill-mid">' + Math.round(val * 100) + '%</span>';
          return '<span class="stage-pill-zero">0%</span>';
        }}
        // Pre-tournament mode
        const pct = Math.round(val * 100);
        if (pct >= 50) return '<span class="stage-pill-high">' + pct + '%</span>';
        if (pct > 0) return '<span class="stage-pill-mid">' + pct + '%</span>';
        return '<span class="stage-pill-zero">0%</span>';
      }};
      ...
      sorted.forEach((t, idx) => {{
        ...
        const champPct = (t.pChampion * 100).toFixed(1) + '%';
        ...
        tr.innerHTML = '<td style="color: var(--neon-gold); font-weight: 800;">#' + (idx + 1) + '</td>' +
                       ...
                       '<td class="' + (t.pChampion > 0 ? 'gold-cell' : 'stage-pill-zero') + '">' + champPct + '</td>';
    ...
    function initPredictor() {{
      ...
      const vSel = document.getElementById('venue-select');
      if (vSel) vSel.addEventListener('change', runSim);
      runSim();
    }}
    ...
    function runSim() {{
      ...
      if (!teamASelect || !teamBSelect || !venueSelect) return;
      ...
      const dA = document.getElementById('display-teamA'); if (dA) dA.textContent = teamA.replace(/-/g, ' ');
      const dB = document.getElementById('display-teamB'); if (dB) dB.textContent = teamB.replace(/-/g, ' ');
      const eA = document.getElementById('elo-tagA'); if (eA) eA.textContent = 'ELO: ' + ra;
      const eB = document.getElementById('elo-tagB'); if (eB) eB.textContent = 'ELO: ' + rb;

      const lA = document.getElementById('lbl-teamA'); if (lA) lA.textContent = teamA.replace(/-/g, ' ') + ' WIN';
      const lB = document.getElementById('lbl-teamB'); if (lB) lB.textContent = teamB.replace(/-/g, ' ') + ' WIN';

      const vA = document.getElementById('val-teamA'); if (vA) vA.textContent = (res.winA * 100).toFixed(1) + '%';
      const vD = document.getElementById('val-draw'); if (vD) vD.textContent = (res.draw * 100).toFixed(1) + '%';
      const vB = document.getElementById('val-teamB'); if (vB) vB.textContent = (res.winB * 100).toFixed(1) + '%';

      const bA = document.getElementById('bar-teamA'); if (bA) bA.style.width = (res.winA * 100) + '%';
      const bD = document.getElementById('bar-draw'); if (bD) bD.style.width = (res.draw * 100) + '%';
      const bB = document.getElementById('bar-teamB'); if (bB) bB.style.width = (res.winB * 100) + '%';

      const exA = document.getElementById('exp-a-val'); if (exA) exA.textContent = res.lambda.toFixed(2);
      const exB = document.getElementById('exp-b-val'); if (exB) exB.textContent = res.mu.toFixed(2);
    ...
    function renderRatingsTable() {{
      const searchInput = document.getElementById('rat-search');
      const search = searchInput ? searchInput.value.toLowerCase() : '';
...
print("✓ Dual-mode index.html generated successfully!")
